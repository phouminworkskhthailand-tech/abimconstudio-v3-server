...
def init_db():
    ...
    # ── Plan Features (feature flags per plan/role) ───────────────────────────
    c.execute("""
    ...
    # ── Idempotent plan_features seed/patch (runs every startup) ─────────────
...
def _sign_thumbnail(thumb: str) -> str:
    ...
    if thumb.startswith("http://") or thumb.startswith("https://"):
        return thumb   # already a full URL — return unchanged
  ...
  #     else:
    ...
    def _download_model(self):
        ...
        if "/" in model_id or model_id.lower().endswith(".skp"):
            ...
            # ── Legacy DB lookup (abimcon-models bucket) ──────────────────
    ...
    def _get_plan_features(self):
        """GET /api/admin/plan-features — return feature matrix for admin UI."""
        if not self._require_admin(): return
        conn = get_db()
        rows = conn.execute(
    ...
    def _update_plan_feature(self):
        """PUT /api/admin/plan-features — toggle one feature for one plan."""
        if not self._require_admin(): return
        body        = self._body()
    ...
    def _add_plan_or_feature(self):
        """POST /api/admin/plan-features — add a new plan or a new feature key.
    ...
    def _get_user_models(self):
        ...
        try:
            ...
            # ── Single-pass: collect ALL bucket keys + sizes ──────────────
            all_objects = {}   # key → size_bytes
            ...
            for key, size in all_objects.items():
                ...
                # Folders named "pro" (case-insensitive) → pro-only models
                ...
                if base_key + ".jpg" in all_keys:
                    ...
                    thumb_url = ""   # no thumbnail → HTML shows placeholder
